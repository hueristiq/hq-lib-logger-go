...
// special — as a logger threshold it suppresses output, while as a message level
// it marks user-facing "print" output. Level implements [encoding.TextMarshaler]
// and [encoding.TextUnmarshaler], so levels round-trip through JSON or YAML
// configuration as their lowercase names ("fatal", "info", and so on).
package levels
